...
def save_library(libraryName, curato, tel, address, email, url, introduce, created):
    try:
        L = TbLibrary.objects.get(libraryname=libraryName, curator=curato, tel=tel, address=address, email=email, url=url,createdate=created, introduce=introduce)
    except:
        L = TbLibrary.objects.update(libraryname=libraryName, curator=curato, tel=tel, address=address, email=email, url=url,createdate=created, introduce=introduce)
        L.save()


def save_manage(name, pwd):
    try:
        M = TbManager.objects.get(name=name,pwd=pwd)
    except:
        M = TbManager.objects.create(name=name,pwd=pwd)
        M.save()
        U = TbPurview.objects.create(id=M.id, sysset=0, readerset=0, bookset=0, borrowback=0,
                                     sysquery=0)
        U.save()

def save_parameter(cost, validity):
    try:
        P = TbParameter.objects.get(cost=cost,validity=validity)
    except:
        P = TbParameter.objects.create(cost=cost, validity=validity)
        P.save()


def save_bookcase(name):
    try:
        B = TbBookcase.objects.get(name=name)
    except:
        B = TbBookcase.objects.create(name=name)
        B.save()

def save_purview(id,sysset,readerset,bookset,borrowback,sysquery):
    TbPurview.objects.filter(id=id).update(sysset=sysset,readerset=readerset,bookset=bookset,borrowback=borrowback,sysquery=sysquery)


def save_book(id, name):
    TbBookcase.objects.filter(id=id).update(name = name)